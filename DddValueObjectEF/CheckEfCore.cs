...
namespace DddValueObjectEF
{
    internal class CheckEfCore
    {
        ...
        static void Write(List<User> users) => users.ForEach(u => Console.WriteLine($"{u.Id} {u.Name} {u.Age}"));

        record UserQueryResult(Guid id, string name, int age, string email, string? extAttr);
    }
}
