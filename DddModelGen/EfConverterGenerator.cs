...
public class EfConverterGenerator : IIncrementalGenerator
{
    ...
    {
        context.RegisterPostInitializationOutput(static ctx => ctx.AddSource($"{Constants.EfConverterAttributeClassName}.cs", EfValueConverterAttributeWriter.Write()));

        var source = context.SyntaxProvider.ForAttributeWithMetadataName(
            $"{Constants.NamespaceName}.{Constants.EfConverterAttributeClassName}", 
    ...
    private static void Emit(SourceProductionContext context, GeneratorAttributeSyntaxContext source)
    {
        ...
        var (attr, index) = Util.GetEfConverterAttributeData(typeSymbol);
        ...
        context.AddSource(Util.CreateFileName(typeSymbol, "EfValueConverter"), code);
    }
